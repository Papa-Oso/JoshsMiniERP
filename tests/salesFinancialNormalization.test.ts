...
test("normalizes eBay product, discounted shipping, tax, and comparable sales", () => {
  const order = toEbayOrder({
    ...
    pricingSummary: {
      total: { value: "41", currency: "USD" },
      priceSubtotal: { value: "30", currency: "USD" },
      deliveryCost: { value: "10", currency: "USD" },
      deliveryDiscount: { value: "-2", currency: "USD" },
      priceDiscount: { value: "-1", currency: "USD" },
      tax: { value: "4", currency: "USD" }
    },
    lineItems: []
  });
      ...
      refunds: [
        {
          ...
          refundDate: "2026-07-11T00:00:00.000Z",
...
test("splits recognized Etsy refund components and leaves unknown adjustments unresolved", () => {
  const complete = etsyRefunds({
    ...
    payment_adjustments: [
      {
        payment_adjustment_id: 3,
        status: "SUCCESS",
        is_success: true,
        total_adjustment_amount: 1200,
        create_timestamp: 1_752_105_600,
        payment_adjustment_items: [
          { payment_adjustment_item_id: 4, adjustment_type: "transaction", amount: 900 },
          { payment_adjustment_item_id: 5, adjustment_type: "shipping", amount: 200 },
          { payment_adjustment_item_id: 6, adjustment_type: "tax", amount: 100 }
        ]
      }
    ]
  });
  ...
  const unresolved = etsyRefunds({
    payment_id: 1,
    receipt_id: 2,
    currency: "USD",
    payment_adjustments: [
      {
        payment_adjustment_id: 7,
        total_adjustment_amount: 500,
        payment_adjustment_items: [{ payment_adjustment_item_id: 8, adjustment_type: "other", amount: 500 }]
      }
    ]
  });
...
test("normalizes Etsy buyer-paid shipping while excluding tax from comparable sales", () => {
  ...
  const order = toEtsyOrder({
    ...
    create_timestamp: 1_752_105_600,
    status: "Completed",
    total_price: money(4_100),
    subtotal: money(3_000),
    total_shipping_cost: money(800),
    total_tax_cost: money(300),
    total_vat_cost: money(0),
    discount_amt: money(100),
    transactions: []
  });
...
test("retrieves Etsy payments through paginated ledger entries and bounded batches", async () => {
  ...
  const fetchMock = async (input: string | URL | Request) => {
    ...
    calls.push(url);
    if (url.pathname.endsWith("/ledger-entries")) {
      const offset = Number(url.searchParams.get("offset"));
      return response(
        offset === 0
          ? { count: 101, results: Array.from({ length: 100 }, (_, index) => ({ entry_id: index + 1 })) }
          : { count: 101, results: [{ entry_id: 101 }] }
      );
    }
    const ids = url.searchParams.get("ledger_entry_ids")?.split(",") ?? [];
    return response({
      results: [
        { payment_id: Number(ids[0]), receipt_id: 10 },
        { payment_id: 1, receipt_id: 10 }
      ]
    });
  };
  const payments = await fetchEtsyPayments(
    "shop",
    "token",
    "key",
    1_700_000_000,
  ...
  assert.equal(calls.filter((url) => url.pathname.endsWith("/ledger-entries")).length, 2);
  assert.equal(calls.filter((url) => url.pathname.endsWith("/ledger-entries/payments")).length, 2);
  assert.equal(
    calls
      .filter((url) => url.pathname.endsWith("/ledger-entries"))
      .every((url) => url.searchParams.get("min_created") === "1700000000"),
    true
  );
  assert.equal(
    calls
      .filter((url) => url.pathname.endsWith("/ledger-entries"))
      .every((url) => url.searchParams.get("max_created") === "1700086400"),
    true
  );
  assert.equal(
    calls.every((url) => !url.pathname.endsWith("/payments") || url.searchParams.has("ledger_entry_ids")),
    true
  );
  assert.equal(payments.length, 2);
});

test("surfaces Etsy ledger provider errors without returning partial payments", async () => {
  const fetchMock = async () => response({ error: "ledger unavailable" }, 503);
  await assert.rejects(
    () => fetchEtsyPayments("shop", "token", "key", 1_700_000_000, 1_700_086_400, fetchMock as typeof fetch),
    /ledger unavailable/
  );
});

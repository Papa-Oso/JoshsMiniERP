...
function ProductTable({ rows, money }: { rows: SalesDashboardPayload["products"]; money:(value:number)=>string }) { return <div className="sales-table-wrap compact"><table><thead><tr><th>Product</th><th>Units</th><th>Revenue</th></tr></thead><tbody>{rows.map(row=><tr key={`${row.sku}:${row.title}`}><td><strong className="top-product-name">{row.title || "Unnamed product"}</strong><small>{row.sku || "Unmatched historical item"}</small></td><td>{row.units}</td><td>{money(row.revenue)}</td></tr>)}</tbody></table>{!rows.length?<p className="empty">No product sales yet.</p>:null}</div>; }

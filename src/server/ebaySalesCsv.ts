...
export async function importEbaySalesCsv(file: string, { dryRun = false } = {}) {
  const orders = parseEbaySalesCsv(await fs.readFile(file, "utf8"));
  if (!dryRun) await upsertSalesOrders("ebay", orders);
  return { dryRun, orders: orders.length, lineItems: orders.reduce((sum, order) => sum + order.lineItems.length, 0), earliestAt: orders.at(-1)?.createdAt ?? null, latestAt: orders[0]?.createdAt ?? null };
}
...
function toOrder(orderId: string, rows: string[][], indexes: Record<string, number>): SalesOrder {
  ...
  return { platform: "ebay", orderId, orderNumber: orderId, createdAt, updatedAt: createdAt, status: [paid ? "PAID" : "", shipped ? "FULFILLED" : ""].filter(Boolean).join(" / "), currency: "USD", grossAmount: total, netAmount: lineItems.reduce((sum, line) => sum + line.amount, 0), countryCode: countryCode(value(first, indexes["Ship To Country"])), regionCode: value(first, indexes["Ship To State"]), itemCount: lineItems.reduce((sum, line) => sum + line.quantity, 0), sourceUrl: `https://www.ebay.com/sh/ord/details?orderid=${encodeURIComponent(orderId)}`, lineItems };
}

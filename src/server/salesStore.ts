...
export type EbayFinancialTransaction = { transactionKey: string; transactionDate: string; type: string; orderId: string; legacyOrderId: string; transactionId: string; referenceId: string; payoutId: string; payoutDate: string; payoutStatus: string; itemId: string; title: string; sku: string; quantity: number; itemSubtotal: number; shippingAmount: number; taxAmount: number; feeAmount: number; grossAmount: number; netAmount: number; currency: string };

export async function upsertEbayTransactions(rows: EbayFinancialTransaction[]) {
  await ensureLegacySalesMigrated();
  await database.write((db) => { ensureSchema(db); db.run("BEGIN"); try { for (const row of rows) db.run(`INSERT INTO ebay_financial_transactions (transaction_key, transaction_date, type, order_id, legacy_order_id, transaction_id, reference_id, payout_id, payout_date, payout_status, item_id, title, sku, quantity, item_subtotal, shipping_amount, tax_amount, fee_amount, gross_amount, net_amount, currency) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(transaction_key) DO UPDATE SET payout_status=excluded.payout_status, net_amount=excluded.net_amount`, [row.transactionKey,row.transactionDate,row.type,row.orderId,row.legacyOrderId,row.transactionId,row.referenceId,row.payoutId,row.payoutDate,row.payoutStatus,row.itemId,row.title,row.sku,row.quantity,row.itemSubtotal,row.shippingAmount,row.taxAmount,row.feeAmount,row.grossAmount,row.netAmount,row.currency]); db.run("COMMIT"); } catch (error) { db.run("ROLLBACK"); throw error; } });
}
